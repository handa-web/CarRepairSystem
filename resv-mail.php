...
$sql .= "WHERE client_id = ? AND resv_id = ? ORDER BY resv_mail_time DESC";
...
<head>
	...
	<title>予約メール履歴 | car_repair予約システム</title>
	<!-- css -->
...
<main id="main">
	...
	<?php if ($list) { ?>
	<p class="list_title">メール履歴</p>
	<?php foreach ($list as $dat) { ?>
	<div class="edit_list">
		<div class="item">
			<div class="title">送信日時</div>
			<div class="data"><?=h($dat['resv_mail_time']);?><span>（<?=h($dat['resv_mail_ok'] ? '送信成功' : '送信失敗');?>）</span></div>
		</div>
		<div class="item">
